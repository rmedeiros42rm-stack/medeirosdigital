exports.handler = async function(event) {
  ...
  for (const r of cleanRecipients) {
    if (!seen.has(r.email)) { seen.add(r.email); unique.push(r); }
  }

  if (!subject || !message) return { statusCode: 400, headers, body: JSON.stringify({ error: 'Assunto e mensagem são obrigatórios.' }) };
  if (!unique.length) return { statusCode: 400, headers, body: JSON.stringify({ error: 'Nenhum e-mail válido encontrado.' }) };
  if (unique.length > 200) return { statusCode: 400, headers, body: JSON.stringify({ error: 'Limite de 200 destinatários por envio.' }) };
  ...
  return { statusCode: 200, headers, body: JSON.stringify({ ok: true, total: unique.length, sent, failed, errors: errors.slice(0, 10) }) };
};
